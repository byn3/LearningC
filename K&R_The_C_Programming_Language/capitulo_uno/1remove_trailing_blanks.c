...
//same as 1.16 and 1.17 but added in remove trail instead of copy function
#include<stdio.h>
#define MAXLINE 1000

int mgetline(char line[],int lim);
int removetrail(char rline[]); //this function added
...
int main(void){
  ...
  while((len=mgetline(line,MAXLINE))>0)
    if(removetrail(line) > 0)//this line was made
      printf("%s",line);
